...
public class TestNeo4JCluster {
	...
	public void createNodeTest() {	
		try {
			try(Transaction tx = graphDb.beginTx()){
                ...
                node.setProperty("channel", "in.ekstep");
				node.setProperty("IL_UNIQUE_ID", uniqueId);
                node.setProperty("IL_SYS_NODE_TYPE", "DATA_NODE");
                node.setProperty("IL_FUNC_OBJECT_TYPE", "Content");
				node.setProperty("name", "Name_" + uniqueId);
				tx.success();
				File file = new File(logsBasePath + "test_graph_event_neo4j.log");
	...
	public static String readFromFile() throws Exception {
		BufferedReader in = new BufferedReader(new FileReader(logsBasePath + "test_graph_event_neo4j.log"));
	    do {
	        ch = in.readLine();
	        tmp.add(ch);
	    } while (ch != null);
	...
	public String createNode(HighlyAvailableGraphDatabase graphDb2) {	
		...
		try {
			try(Transaction tx = graphDb.beginTx()){
				...
				node.setProperty("IL_SYS_NODE_TYPE", "DATA_NODE");
				node.setProperty("IL_FUNC_OBJECT_TYPE", "Content");
				node.setProperty("channel", "test_channel");
